...
> **MVP 0.12：阶段 A-D 框架、M5 证据门禁、M6 当前 Shadow、M7 受控决策案例和 M8 结果评价均已完成快速 MVP。**

它已经不是静态页面原型：代码具备本地数据库访问、API、iFinD 与 LLM 网关、公告归档、PDF 解析、程序化财务事实、不可变证据快照、统一研究运行、手工 Claim 重评、全市场因子快照和候选池。M5 增加真实证据门禁；M6 增加模型注册、滚动 OOS 复核和当前 Shadow；M7 增加不可变决策案例、三维评分、确定性准入、风险边界与追加式人工审批；M8 增加不可变结果快照、完整周期收益、基准超额、MAE 与跨案例归因。当前自动化测试基线为 **134 项通过**。这里的“可用”仍只表示单用户开发环境中的技术验收，不代表生产可用。

它还不是完整的“AI 研究助手”：Thesis 监控与因子快照均由用户手工触发；量化层只覆盖价格、波动和成交活跃度，不含行业中性化、估值、基本面因子和回测；行业、同行比较及真实财报样本的长期抽取准确率仍待建设或验收。
...
| 横截面量化辅助 | 快速 MVP | 按 `as_of` 生成版本化全市场快照，提供质量隔离、排序、组合筛选和研究候选池；无行业中性化、估值因子、回测和交易 |
...
- 没有行业中性化、ST/退市状态、涨跌停可交易性、估值、基本面因子、回测和交易成本。

### 6.8 前端工作台

入口：`app/static/index.html`、`app/static/app.js`、`app/static/app.css`

当前功能：

- 一级导航已按用户任务收敛为“每日决策、公司研究、量化实验室、评价与审计”四个工作区；每日决策为默认首页；
...
- 桌面端和 375px 移动端已验证图表绘制、响应式布局和无横向溢出，页面控制台无错误；首笔成交日起算合同加入回归后，`quant` 环境全量回归为 **134 passed**。
