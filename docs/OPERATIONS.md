...
| 数据库 | 职责 |
|---|---|
| `data/research_state.db` | 研究、证券主数据、公告、财务、决策、Thesis、运行事件与 iFinD 审计 |
| `data/quant_research.db` | 因子、模型、Shadow、评价、回测与因子发布 |
| `data/paper_trading.db` | 模拟账户、策略、研究批次、订单、持仓、净值、行情缓存与每日批次 |

首次启动拆分版本前应备份 `research_state.db`。启动时 `0014` 和 `0015` 会创建并填充
`paper_trading.db`，复制过程可重复执行。迁移后应逐表
核对行数，并以 API 的账户、持仓和订单结果作为业务校验。

回滚必须同时恢复上一版应用代码和启动前的数据库备份。不要仅删除
`paper_trading.db` 后继续运行新版本；新版本的后续写入不会同步回旧库。

量化拆分首次启动时由 `0017` 至 `0021` 创建和填充 `quant_research.db`。迁移后因子、
模型、Shadow、评价、回测和发布的新写入只进入量化库。正式模式随后由 `0022` 删除
研究库中的旧模拟盘和量化表；启动时会从研究库批量同步 `security_master` 投影，
不应直接修改量化库中的证券主数据。回滚必须同时恢复旧代码和启动前备份。
