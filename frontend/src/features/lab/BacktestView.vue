...
<template>
  ...
  <section class="content-band white"><AsyncState :loading="latest.isPending.value" :error="latest.error.value" :empty="!data"><div class="section-heading"><div><span>NAV CURVE</span><b>扣费后策略净值</b></div><small>{{ navPoints.length }} 个交易日</small></div><LineChart :points="navPoints" value-label="策略净值" /><div class="section-heading" style="margin-top:24px"><div><span>REBALANCE LEDGER</span><b>最近调仓</b></div><small>{{ data?.run?.backtest_id }}</small></div><div class="data-table-wrap"><table class="data-table"><thead><tr><th>信号/成交</th><th>买入</th><th>卖出</th><th>受阻</th><th>换手</th><th>成本</th></tr></thead><tbody><tr v-for="item in (data?.rebalances||[]).slice(-30).reverse()" :key="item.execution_date"><td>{{ item.signal_date }} / {{ item.execution_date }}</td><td>{{ item.executed_buy_count }}</td><td>{{ item.executed_sell_count }}</td><td>{{ item.blocked_buy_count + item.blocked_sell_count }}</td><td>{{ pct(item.turnover) }}</td><td>{{ money(item.explicit_cost + item.slippage_cost) }}</td></tr></tbody></table></div><div class="section-heading" style="margin-top:24px"><div><span>BOUNDARIES</span><b>强制边界</b></div></div><div class="list"><article v-for="item in data?.run?.limitations||[]" :key="item" class="list-item"><p>{{ item }}</p></article></div></AsyncState></section>
